...
if [ -z "$VSIP" ] || [ -z "$VSPORT" ]; then
  ...
  REACHED=""
  for ip in $VSIP; do
    # -S so the status line lands in the output: the editor answers 401 without
    # a token, and a 401 is still REACHED. Only the absence of any HTTP reply
    # means the packet did not arrive.
